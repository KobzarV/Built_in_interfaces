namespace Studentns
{
    public class Student : IComparable<Student>
    {
        ...
        public int CompareTo(Student other) // зрівнюємо по середньому балу за домашки
        {
            if (other == null)
                return 1; // Інший об'єкт є null, отже, поточний об'єкт більший
            if (Math.Round(Homeworks.Average(), 2) > Math.Round(other.Homeworks.Average(), 2))
                return 1;
            else if (Math.Round(Homeworks.Average(), 2) < Math.Round(other.Homeworks.Average(), 2))
                return -1;
            else
                return 0; // Середні бали однакові
        }

        // Вкладений клас для сортування за прізвищем
        ...
        // Вкладений клас для сортування за екзаменами
        public class ExamsComparer : IComparer<Student>
        {
            public int Compare(Student x, Student y)
            {
                return Math.Round(x.Exams.Average(), 2).CompareTo(Math.Round(y.Exams.Average(), 2));
            }
        }
    }
